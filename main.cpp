...
void Spawning (vector<EnemyObject> Vector_Enemy, SDL_Renderer* des)
{
     EnemyObject enemy;
   int random=rand()%100;
   if (random<=10)
    {   enemy.LoadImg("img/enemy.png",des);
        enemy.set_clips();
        Vector_Enemy.push_back(enemy);
    }
    cout<<Vector_Enemy.size()<<endl;
   for (auto& enm:Vector_Enemy)
   {
       enm.Show(des);
   }
}
